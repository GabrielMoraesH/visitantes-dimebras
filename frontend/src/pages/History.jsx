...
export default function History() {
  ...
  const isAdmin =
    user?.isAdmin === true || Number(user?.sub) === 1 || Number(user?.id) === 1;
  ...
  async function carregar(p = page, l = limit) {
    ...
    try {
      ...
      if (cpf) params.set("cpf", onlyDigits(cpf));
      ...
      <div className="history-container">
        <div className="history-header">
          ...
          <div className="history-headerActions">
            <button className="h-btn h-btn-ghost" onClick={() => carregar(1, limit)}>
              ATUALIZAR
            </button>
            <button className="h-btn h-btn-ghost" onClick={() => navigate("/checkin")} type="button">
              VOLTAR
            </button>
          </div>
        ...
        <div className="history-card">
                ...
                {items.length === 0 ? (
                  ...
                  items.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td>{v.attendedBy || "-"}</td>
                      <td>{v.checkinByUser?.username || "-"}</td>
                      <td>{v.checkoutByUser?.username || "-"}</td>
                      <td>{v.branchName || v.branch?.name || "-"}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          >
            <div style={{ fontSize: 13, fontWeight: 700, color: "#5f7a89" }}>
